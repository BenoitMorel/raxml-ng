...
Optimizer::Optimizer (const Options &opts) :
    _lh_epsilon(opts.lh_epsilon), _spr_radius(opts.spr_radius), _spr_cutoff(opts.spr_cutoff), _use_tbnni(opts.use_tbnni)
{
}
...
double Optimizer::optimize_topology_tbnni(TreeInfo& treeinfo, CheckpointManager& cm)
{
...
//  treeinfo->counter = 0;
  ...
  tbnni_round_params tbnni_params;
  int plop;
  for (plop = 0; plop < 5; ++plop) 
  {
    loglh = treeinfo.tbnni_round(tbnni_params);
    LOG_PROGRESS(loglh) << "TBNNI  iterations: " << plop << endl;
  }
...
double Optimizer::optimize_topology(TreeInfo& treeinfo, CheckpointManager& cm)
{
  if (_use_tbnni) 
  {
    return optimize_topology_tbnni(treeinfo, cm);
  }
  const double fast_modopt_eps = 10.;
  const double interim_modopt_eps = 3.;
